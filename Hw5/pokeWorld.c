...
bool validate_PC_Move(terType_t ter, int x, int y)
{
...
char get_NPC_Char(charType_t type)
{
  switch (type)
  {
  case char_hiker:
...
void gameLoop(map_t *terMap)
{
  ...
  world.hikerMap = malloc(sizeof(distMap_t));
  world.hikerMap->npcType = char_hiker;

  world.rivalMap = malloc(sizeof(distMap_t));
  ...
  while ((c = heap_remove_min(&h)) && !quitGame)
  {
    ...
    case char_pc:
      ...
      case 't':
        ...
        {
          character_t npc = terMap->characterArray[i];
          ...
          char *verticalDirection, *horizontalDirection;
          ...
          mvprintw(22 + i, 0, "%c, %d %s and %d %s", npcChar, verticalPos, verticalDirection, horizontalPos, horizontalDirection);
        }
        refresh();
    ...
    case char_rival:
      ...
      distMap_t *tempDistMap = world.rivalMap;
