...
// Function Protoypes (Not all functions listed, only the ones needed)
...
void generateTrainerPokemon(character *trainer);
...
void generateMap(map *map, int north, int east, int south, int west)
{
    ...
    map->characterArray = new character[numTrainers + 1];
...
InGamePokemon spawnPokemon()
{
    ...
    InGamePokemon currentPokemon;
    currentPokemon.id = p.id;

    currentPokemon.pokemonName = p.identifier;
    for (int i = 0; i < (int)pokemonTypesVector.size(); i++)
    {
        if (currentPokemon.id == pokemonTypesVector[i].pokemon_id)
        {
            currentPokemon.type_id = pokemonTypesVector[i].type_id;
            break;
        }
    }
    ...
    for (int i = 0; i < (int)pokemonStatsVector.size(); i++)
    {
        if (pokemonStatsVector[i].pokemon_id == p.id)
        {
            ...
            case 1:
                ...
                currentPokemon.max_hp = pokemonStatsVector[i].base_stat + pokeIV;
                break;
            case 2:
...
double calculateDamage(InGamePokemon p, Moves m)
{
    int power = m.power;
    int level = p.level;
    int attack = p.attack;
    int defense = p.defense;
    double random = (rand() % 16 + 85) / (double)100;
    int type = 1; // Change later for 1.09
    double critical;
    double STAB;

    rand() % 100 < CRIT_CHANCE ? critical = 1.5 : critical = 1.0;
    m.type_id == p.type_id ? STAB = 1.5 : STAB = 1.0;

    double damage = (((((2 * level) / 5) + 5) * power * (attack / defense) / 50) + 5) * critical * random * STAB * type;

    return damage;
}

void wildPokemonEncounter(InGamePokemon wildPokemon)
{
    int32_t userKey = -1;
    ...
    {
        clear();
        mvprintw(0, 0, "Encountered a wild Level %d %s! (Esc to exit)", wildPokemon.level, wildPokemon.pokemonName.c_str());
        mvprintw(1, 0, "HP: %d", wildPokemon.hp);
        mvprintw(3, 0, "1) Fight");
        mvprintw(4, 0, "2) Inventory Bag");
        mvprintw(5, 0, "3) Run - Attempt to Flee the Battle");
        ...
        case '1':
            ...
            clear();
            mvprintw(0, 0, "%s I chooose you! ('q') to go back", player->pokemonList[selectedPokemon].pokemonName.c_str());
            mvprintw(1, 0, "Level: %d", player->pokemonList[selectedPokemon].level);
            mvprintw(2, 0, "HP: %d", player->pokemonList[selectedPokemon].hp);
            mvprintw(4, 0, "Available Moves:");

            int i;

            // Checks if pokemon has no moves to display
            if (!player->pokemonList[selectedPokemon].allPokemonMoves.size())
            {
                ...
                i = 1;
                mvprintw(5, 0, "No Moves! This is a Display (aesthetic) Pokemon! Better not take it into battle!");
            }
            else
            {
                // Pokemon has moves but are not available currently due to low level
                if (!player->pokemonList[selectedPokemon].availableMoves.size())
                {
                    i = 1;
                    mvprintw(6, 0, "No moves available yet! Level up this Pokemon to unlock new moves!");
                }
                // Pokemon has moves available to display
                else
                {
                    for (i = 0; i < (int)player->pokemonList[selectedPokemon].availableMoves.size(); i++)
                    {
                        mvprintw(5 + i, 0, "%d) %s", i + 1, player->pokemonList[selectedPokemon].availableMoves[i].identifier.c_str());
                    }
                }
            }
            refresh();
            // Allow user to select an available move if any
            while (userKey != 'q' && !validSelection)
            {
                int userSelection = -1;
                userKey = getch();
                if (userKey == '1')
                {
                    userSelection = 0;
                    if (userSelection >= 0 && userSelection <= (int)player->pokemonList[selectedPokemon].availableMoves.size())
                    {
                        pcMoves = player->pokemonList[selectedPokemon].availableMoves[0];
                        validSelection = true;
                    }
                    else
                    {
                ...
                if (userKey == '2')
                {
                    ...
                    if (userSelection > 0 && userSelection <= (int)player->pokemonList[selectedPokemon].availableMoves.size())
                    {
                        pcMoves = player->pokemonList[selectedPokemon].availableMoves[1];
                        validSelection = true;
                    }
                    else
                    {
                ...
                // Valid move selected, calculate damage delt
                if (userSelection != -1)
                {
                    clear();

                    double damage = calculateDamage(player->pokemonList[selectedPokemon], pcMoves);

                    mvprintw(0, 0, "%s use %s!", player->pokemonList[selectedPokemon].pokemonName.c_str(),
                             pcMoves.identifier.c_str());
                    mvprintw(1, 0, "Delt %lf damage!", damage);
                    ...
                    {
                        clear();
                        mvprintw(0, 0, "You defeated %s! Would you like to capture this Pokemon (y/n)?", wildPokemon.pokemonName.c_str());
                        refresh();
                        userKey = getch();
                        if (userKey == 'y')
                        {
                            if ((int)player->pokemonList.size() < 6)
                            {
                                player->pokemonList.push_back(wildPokemon);
                                mvprintw(1, 0, "Current Pokedex:");
                                mvprintw(2, 0, "----------------");
                                for (int i = 0; i < (int)player->pokemonList.size(); i++)
                                {
                                    mvprintw(3 + i, 0, "%d) %s", i + 1, player->pokemonList[i].pokemonName.c_str());
                                }
                            }
                            else
                            {
                                mvprintw(2, 0, "Pokedex Full!");
                            }
                        }
                        refresh();
                        usleep(1000000);
                    ...
                    if (!(int)wildPokemon.availableMoves.size())
                    {
                        clear();
                        mvprintw(0, 0, "%s has no available moves!", wildPokemon.pokemonName.c_str());
                        refresh();
                        usleep(2000000);
                        break;
                    }
                    Moves wildPokeRandMove = wildPokemon.availableMoves[rand() % (int)wildPokemon.availableMoves.size()];
                    double wildPokeDMG = calculateDamage(wildPokemon, wildPokeRandMove);
                    player->pokemonList[selectedPokemon].hp -= damage;
                    clear();
                    mvprintw(0, 0, "%s uses %s! It delt %lf damage!", wildPokemon.pokemonName.c_str(),
                             wildPokeRandMove.identifier.c_str(), wildPokeDMG);
                    refresh();
                    usleep(1000000);
                    mvprintw(2, 0, "%s HP: %d", player->pokemonList[selectedPokemon].pokemonName.c_str(),
                             player->pokemonList[selectedPokemon].hp);
                    refresh();
                    usleep(1000000);
                }
            }
        ...
        case '2':
            clear();
            mvprintw(0, 0, "Inventory ('q' to go back)");
            mvprintw(1, 0, "--------------------------");
            mvprintw(2, 0, "1) Heal (Restore Pokemon to full HP)");
            refresh();
            while (1)
            {
                userKey = getch();
                ...
                {
                    clear();
                    mvprintw(0, 0, "Select a Pokemon to heal!");
                    for (int i = 0; i < (int)player->pokemonList.size(); i++)
                    {
                        mvprintw(1 + i, 0, "%d) %s", i + 1, player->pokemonList[i].pokemonName.c_str());
                    }

                    int userSelection; // Index for the pokemon vector
                    switch (userKey = getch())
                    {
                    case '1':
                        userSelection = 0;
                        break;
                    ...
                    clear();
                    if (userSelection <= 0 && userSelection >= (int)player->pokemonList.size())
                    {
                        mvprintw(0, 0, "Invalid Input!");
                        break;
                    }
                    // Heal pokemon to max HP
                    player->pokemonList[userSelection].hp = player->pokemonList[userSelection].max_hp;
                    mvprintw(0, 0, "Healed %s to %d HP!", player->pokemonList[userSelection].pokemonName.c_str(),
                             player->pokemonList[userSelection].hp);
                    refresh();
                    usleep(2000000);
                    break;
                }
        ...
        case '3':
            clear();
            mvprintw(0, 0, "Attempting to flee");
            refresh();
            usleep(500000);
            return;
        case '4':
            ...
            selectedPokemon = userSelection;
            mvprintw(0, 0, "Selected %s!", player->pokemonList[selectedPokemon].pokemonName.c_str());
            refresh();
            usleep(1000000);
...
void trainerBattle(map *terMap, character *c, int xChange, int yChange)
{
    int tempX, tempY;
    ...
    for (j = 0; j < numTrainers + 1; j++)
    {
        ...
        {
            trainer = terMap->characterArray[j];
            break;
        }
    }
    ...
    if (!(terMap->characterArray[j].beenDefeated))
    {
        ...
        while (1)
        {
            ...
            clear();
            mvprintw(0, 0, "Entering Trainer battle! This Trainer has %d Pokemon! (Esc)", (int)trainer.pokemonList.size());
            mvprintw(1, 0, "Currently battling a Level %d %s!", trainer.pokemonList[currentTrainerPokemon].level,
                     trainer.pokemonList[currentTrainerPokemon].pokemonName.c_str());
            mvprintw(2, 0, "HP: %d", trainer.pokemonList[currentTrainerPokemon].hp);

            mvprintw(4, 0, "1) Fight");
            mvprintw(5, 0, "2) Inventory Bag");
            mvprintw(6, 0, "3) Run - Attempt to Flee the Battle");
            mvprintw(7, 0, "4) Switch Pokemon");
                ...
                while (userKey != 'q' && !validSelection)
                {
                    ...
                    if (userSelection != -1)
                    {
                        ...
                        mvprintw(0, 0, "%s use %s!", player->pokemonList[selectedPokemon].pokemonName.c_str(),
                                 pcMoves.identifier.c_str());
                        mvprintw(1, 0, "Delt %lf damage!", damage);
                        trainer.pokemonList[currentTrainerPokemon].hp -= damage;
                        refresh();
                        usleep(1000000);

                        if (trainer.pokemonList[currentTrainerPokemon].hp <= 0)
                        {
                            clear();
                            mvprintw(0, 0, "You knocked out %s!",
                                     trainer.pokemonList[currentTrainerPokemon].pokemonName.c_str());
                            if (currentTrainerPokemon + 1 < (int)trainer.pokemonList.size())
                            {
                            ...
                            {
                                mvprintw(2, 0, "You have defeated this trainer!");
                        ...
                        if (!(int)trainer.pokemonList[currentTrainerPokemon].availableMoves.size())
                        {
                            clear();
                            mvprintw(0, 0, "%s has no available moves!", trainer.pokemonList[currentTrainerPokemon].pokemonName.c_str());
                            refresh();
                            usleep(2000000);
                            break;
                        }
                        Moves trainerPokeMove =
                            trainer.pokemonList[currentTrainerPokemon].availableMoves[rand() %
                                                                                      (int)trainer.pokemonList[currentTrainerPokemon].availableMoves.size()];
                        double trainerPokeDMG = calculateDamage(trainer.pokemonList[currentTrainerPokemon], trainerPokeMove);
                        player->pokemonList[selectedPokemon].hp -= trainerPokeDMG;
                        clear();
                        if (player->pokemonList[selectedPokemon].hp <= 0)
                        {
                            if (selectedPokemon + 1 < (int)player->pokemonList.size())
                            {

                                mvprintw(0, 0, "%s has been knocked out! %s I choose you!",
                                         player->pokemonList[selectedPokemon].pokemonName.c_str(),
                                         player->pokemonList[selectedPokemon + 1].pokemonName.c_str());
                                selectedPokemon++;
                                refresh();
                                usleep(2000000);
                            }
                            ...
                            {
                                mvprintw(0, 0, "You are out of Pokemon!");
                                refresh();
                                usleep(2000000);
                                return;
                            }
                        }
                        mvprintw(0, 0, "%s uses %s! It delt %lf damage!", trainer.pokemonList[currentTrainerPokemon].pokemonName.c_str(),
                                 trainerPokeMove.identifier.c_str(), trainerPokeDMG);
                        refresh();
                        usleep(1000000);
                        mvprintw(2, 0, "%s HP: %d", player->pokemonList[selectedPokemon].pokemonName.c_str(),
                                 player->pokemonList[selectedPokemon].hp);
...
void gameLoop(map *terMap)
{
    ...
    for (int i = 0; i < numTrainers + 1; i++)
    {
        ...
        // For all trainers, generate their pokemon
        if (i > 0)
        {
            generateTrainerPokemon(&terMap->characterArray[i]);
        }
    }
    ...
    while ((c = (character *)heap_remove_min(&h)) && !quitGame)
    {
        ...
        case char_pc:
        {
            ...
            */
            ...
            {
                trainerBattle(terMap, c, xChange, yChange);
                c->cost += pathCost(c->currTerrain, c->characterType);
                heap_insert(&h, c);
                break;
            }
            ...
            */
            ...
            {
                int pokeSpawnChance = rand() % 100 + 1;
                // 10% chance for Pokemon to spawn in tall grass, exit if not
                if (pokeSpawnChance <= 10 && pokeSpawnChance > 0)
                {
                    InGamePokemon wildPokemon = spawnPokemon();
                    wildPokemonEncounter(wildPokemon);
                }
            }
    ...
    for (int i = 0; i < WORLD_DIM; i++)
    {
        for (int j = 0; j < WORLD_DIM; j++)
        {
            if (world.mapArray[i][j] != NULL)
            {
                ...
                delete[] world.mapArray[i][j]->characterArray;
...
{
    InGamePokemon option1 = spawnPokemon();
    InGamePokemon option2 = spawnPokemon();
    InGamePokemon option3 = spawnPokemon();
    ...
    while (!selectedPokemon)
    {
        ...
        mvprintw(0, 0, "Starting Pokemon");
        mvprintw(1, 0, "----------------");
        mvprintw(2, 0, "1) %s", option1.pokemonName.c_str());
        mvprintw(3, 0, "2) %s", option2.pokemonName.c_str());
        mvprintw(4, 0, "3) %s", option3.pokemonName.c_str());
        mvprintw(5, 0, "Select Pokemon '1' '2' or '3'");
        refresh();

        switch ((userKey = getch()))
        {
        case '1':
            player->pokemonList.push_back(option1);
            selectedPokemon = true;
            break;
        case '2':
            player->pokemonList.push_back(option2);
            selectedPokemon = true;
            break;
        case '3':
            player->pokemonList.push_back(option3);
            selectedPokemon = true;
            break;
        default:
            mvprintw(6, 0, "Invalid input, try again!");
            break;
        }
    }
}
...
void generateTrainerPokemon(character *trainer)
{
    // Gives trainer at least one pokemon
    InGamePokemon temp = spawnPokemon();
    trainer->pokemonList.push_back(temp);

    int additionalPokemon = 1;
    while (additionalPokemon < 6)
    {
        // 40% chance at not getting an additional pokemon.
        if ((rand() % 100) < 40)
        {
            return;
        }
    ...
    // clear();
    // for (int i = 0; i < trainer->pokemonList.size(); i++)
    // {
    //     mvprintw(i, 0, "%s", trainer->pokemonList[i].pokemonName.c_str());
    // }
