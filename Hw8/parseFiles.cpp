...
void parsePokemonTypes(std::string filepath)
{
    filepath.append("pokemon_types.csv");
    std::ifstream file;
    std::string line;
    file.open(filepath);
    ...
    while (std::getline(file, pokemon_id, ','))
    {
        ...
        // std::cout << pokemon_id << "," << type_id << "," << slot << std::endl;

        PokemonTypes temp = PokemonTypes(std::stoi(pokemon_id), std::stoi(type_id), std::stoi(slot));
        pokemonTypesVector.push_back(temp);
    }
}